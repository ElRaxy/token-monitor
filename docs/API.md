...
`limits.providers[].provider` is one of `claude`, `codex`, `cursor`, `antigravity`, `opencode`, `openrouter`, `deepseek`, `minimax`, `mimo`, `grok`, `copilot`, `kiro`, `commandcode`, `zai`, `zaiteam`, `volcengine`, `qoder`, `workbuddy`, `kimi`, `ollama`, `trae`, or `thirdparty`.

### CodexBar limits provenance

Provider rows sourced from dashboard-v1 carry optional, additive provenance fields. `producer` is `codexbar`; `producerVersion` is the bounded CodexBar version reported by the dashboard host; `producedAt` is the dashboard snapshot timestamp; and `staleAfterMs` is that snapshot's freshness allowance in milliseconds. `sourceDeviceId` identifies the Token Monitor device that published the normalized row after collection. These fields describe where a limits row came from without replacing the provider's existing `source` semantics (`oauth`, `cli`, `web`, `rpc`, `local`, or `api`).

A device update containing only fresh limits sets the top-level `limitsOnly` marker. The hub then preserves the device's existing usage periods, sessions, projects, history, `updatedAt`, and health data while replacing only the limits projection. A normal usage update omits this marker.

Compatibility is additive: every CodexBar field above is optional, older producers may omit it, and consumers must ignore unknown future fields. The bearer, raw dashboard response, provider credentials, costs, usage, sessions, and dashboard identity never enter this API payload.

CodexBar-delegable provider IDs are `claude`, `codex`, `opencode`, `cursor`, `antigravity`, `kimi`, `grok`, `copilot`, `commandcode`, `mimo`, `zai`, `kiro`, `qoder`, `deepseek`, `openrouter`, `minimax`, `volcengine`, and `ollama`.
